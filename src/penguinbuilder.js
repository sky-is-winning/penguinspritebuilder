...
async function buildAnimation(options) {
    ...
    let penguinArray = Object.values(penguinModel).filter((item) => item != 0);
...
async function processFrame(frame, penguinModel, cacheString) {
    ...
    async function processSubframe(f) {
        ...
        while (filename.split("_")[1].length < 4) {
            filename =
                filename.split("_")[0] + "_" + "0" + filename.split("_")[1];
        }
    ...
    async function createGif() {
        ...
        const stream = pngFileStream(
            ...
            .pipe(
                encoder.createWriteStream({ repeat: 0, delay: 42, quality: 10 })
            )
            .pipe(
                fs.createWriteStream(`${outputDir}/${cacheString}/${frame}.gif`)
            );

        stream.on("finish", () => {
            for (let file of frameList) {
                try {
                    fs.unlinkSync(`${outputDir}/${cacheString}/${file}`);
                } catch (err) {}
            }
            return true;
        });
